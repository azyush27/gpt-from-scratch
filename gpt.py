...
class Bigramlanguagemodel(nn.Module):
    def __init__(self,vocab_size):
        super(). __init__()
        #each directly reads off the logits for the next token from a lookup table 
        self.token_embedding_table = nn.Embedding(vocab_size , vocab_size)

    def forward(self,idx,targets=None):
            #idx and targets are both (B,T ) tensors of integers # for example if b t = xb,yb then the tensors dimensions will be (4,8)
    ...
    def generate (self,idx,max_new_tokens):
        #idx is (VB,T ) array of indices in the current context
